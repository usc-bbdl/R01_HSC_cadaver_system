// This time we will try to use the already defined include files
// and manually set the configuration for the program directories
...
{    
	...
	count = 0;
	global_desired_position[0]=0.0;
	global_desired_position[1]=0.0;
	global_desired_position[2]=0.0;
    do
	{
		printf("\n%d", count);
		global_desired_position[0] = 0.0;// 20*sin((count)*6.28/360.0/4.0);
		global_desired_position[1] = 0.0;//20*sin(0.5*(count)*6.28/360.0/4.0);
		global_desired_position[2] = 20*cos((count)*6.28/360.0);

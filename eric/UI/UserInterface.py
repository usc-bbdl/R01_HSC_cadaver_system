...
class UserInterface(KQMainWindow):
    ...
    def __init__(self, app, locale, splash, plugin, noOpenAtStartup, restartArguments):
        ...
        @param app reference to the application object (KQApplication)
        ...
        self.connect(self.debuggerUI, SIGNAL('appendStdout'),
                     self.appendToStdout)
        ...
        self.connect(self, SIGNAL('preferencesChanged'),
                     self.debugViewer.preferencesChanged)
...
##        sys.stderr = self.stderr
                ...
                try:
                    ...
                    KQMessageBox.warning(None,
                        ...
                        self.trUtf8("""<p>The Qt Style Sheet file <b>%1</b> could"""
                            ...
                            .arg(styleSheetFile).arg(str(msg)),
    ...
    def __createDockWindowsLayout(self, debugServer):
        ...
        self.windows = [self.projectBrowserDock, None, self.debugViewerDock, 
            ...
            self.multiProjectBrowserDock, None]
    ...
    def __createSidebarsLayout(self, debugServer):
        ...
        self.leftSidebar = E4SideBar(E4SideBar.West, delay)
        
        # Create the bottom sidebar
        self.bottomSidebar = E4SideBar(E4SideBar.South, delay)
    ...
    def __initActions(self):
        ...
        self.newWindowAct = E4Action(self.trUtf8('New Window'),
                ...
                0, self, 'new_window')
        self.newWindowAct.setStatusTip(self.trUtf8('Open a new eric4 instance'))
        self.newWindowAct.setWhatsThis(self.trUtf8(
            """<b>New Window</b>"""
            """<p>This opens a new instance of the eric4 IDE.</p>"""
        ))
        ...
        if Utilities.isWindowsPlatform():
            designerExe = "%s.exe" % Utilities.generateQtToolName("designer")
        elif Utilities.isMacPlatform():
            designerExe = unicode(Utilities.getQtMacBundle("designer"))
        else:
        ...
        if Utilities.isWindowsPlatform():
            linguistExe = "%s.exe" % Utilities.generateQtToolName("linguist")
        elif Utilities.isMacPlatform():
            linguistExe = unicode(Utilities.getQtMacBundle("linguist"))
        else:
        ...
        self.webBrowserAct = E4Action(self.trUtf8('eric4 Web Browser'),
    ...
    def __newWindow(self):
        ...
        if not Preferences.getUI("SingleApplicationMode"):
            # start eric4 without any arguments
            program = sys.executable
            eric4 = os.path.join(getConfig("ericDir"), "eric4.py")
            args = [eric4]
            QProcess.startDetached(program, args)
    ...
    def __UIPreviewer(self,fn=None):
        ...
        viewer = os.path.join(getConfig("ericDir"), "eric4_uipreviewer.py")
    ...
    def __TRPreviewer(self, fileNames = None, ignore = False):
        ...
        viewer = os.path.join(getConfig("ericDir"), "eric4_trpreviewer.py")
    ...
    def __sqlBrowser(self):
        ...
        browser = os.path.join(getConfig("ericDir"), "eric4_sqlbrowser.py")
        ...
        if not pythonDocDir.startswith("http://") and \
           not pythonDocDir.startswith("https://"):
            ...
            if not home.endswith(".chm"):
                if Utilities.isWindowsPlatform():
                    home = "file:///" + unicode(Utilities.fromNativeSeparators(home))
                else:
                    home = "file://" + home
        else:
            home = pythonDocDir
    ...
    def __showQt4Doc(self):
        ...
        if not qt4DocDir:
            ...
            if not qt4DocDir:
                qt4DocDir = os.path.join(unicode(Preferences.getQt4DocDir()), "html")
    ...
    def __versionsDownloadDone(self):
        ...
        if reply.error() == QNetworkReply.NoError:
            ioEncoding = str(Preferences.getSystem("IOEncoding"))
            versions = unicode(reply.readAll(), ioEncoding, 'replace').splitlines()
    ...
    def __versionCheckResult(self, versions):
        ...
        try:
            if "-snapshot-" in Version:
                ...
                if versions[2][0] == "4" and versions[2] > Version:
    ...
    def checkProjectsWorkspace(self):
        ...
        if workspace.isEmpty():
            ...
            workspace = KQFileDialog.getExistingDirectory(
                None,

...
class BrowserModel(QAbstractItemModel):
    ...
    def __init__(self, parent = None):
        ...
        self.connect(self.watcher, SIGNAL("directoryChanged(const QString&)"), 
    ...
    def directoryChanged(self, path):
        ...
        for itm in self.watchedItems[path]:
            ...
            for f in entryInfoList:
                fpath = unicode(Utilities.toNativeSeparators(f.absoluteFilePath()))
                childFound = False
            ...
            if len(entryInfoList) != itm.childCount():
                for row in range(oldCnt - 1, -1, -1):
                    ...
                    entryFound = False
                    for f in entryInfoList:
                        if unicode(f.absoluteFilePath()) == childname:
                            entryFound = True
    ...
    def programChange(self, dirname):
        ...
        @param dirname name of the directory containing the file (string or QString)
        """
        if self.progDir:
        ...
        if len(entryInfoList) > 0:
            ...
            for f in entryInfoList:
                if f.isDir():
                    ...
                    if not fileFilters.isEmpty():
                        fn = unicode(f.fileName())
                        if any([fnmatch.fnmatch(fn, unicode(ff).strip()) \

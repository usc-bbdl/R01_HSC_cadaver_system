...
class Subversion(VersionControl):
    ...
    def __init__(self, plugin, parent=None, name=None):
        ...
        self.rx_status1 = QRegExp('(.{8})\\s+([0-9-]+)\\s+([0-9?]+)\\s+(\\S+)\\s+(.+)')
    ...
    def vcsRevert(self, name):
        ...
        self.addArguments(args, self.options['global'])
        if type(name) is types.ListType:
            if len(name) == 1 and os.path.isdir(name[0]):
    ...
    def svnLogLimited(self, name):
        ...
        @param name file/directory name to show the log of (string)
        """
        noEntries, ok = KQInputDialog.getInt(\
            None,
    ...
    def svnAddToChangelist(self, names):
        ...
        @param names name or list of names of file or directory to add
            (string or QString)
        """
        clname, ok = KQInputDialog.getItem(\
            None,
    ...
    def svnGetChangelists(self):
        ...
        if procStarted:
            ...
            if finished and process.exitCode() == 0:
                output = \
                    unicode(process.readAllStandardOutput(),
                            Preferences.getSystem("IOEncoding"),
                            'replace')
                if output:
                    for line in output.splitlines():
                        if rx_changelist.exactMatch(line):
                            changelist = unicode(rx_changelist.cap(1))

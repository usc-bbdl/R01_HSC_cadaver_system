...
class Editor(QsciScintillaCompat):
    ...
    def __init__(self, dbs, fn = None, vm = None,
        ...
        self.connect(self, SIGNAL("linesChanged()"), self.__resizeLinenoMargin)
    ...
    def saveFile(self, saveas = False, path = None):
        ...
        if saveas or self.fileName is None:
            ...
            if path is None or QString(path).isEmpty():
                path = Preferences.getMultiProject("Workspace") or Utilities.getHomeDir()
            if unicode(self.fileName):
                filterPattern = "(*%s)" % os.path.splitext(unicode(self.fileName))[1]
                for filter in Lexers.getSaveFileFiltersList(True):
                    if filterPattern in filter:
                        selectedFilter = QString(filter)
                        break
                else:
                    selectedFilter = Preferences.getEditor("DefaultSaveFilter")
            else:
                selectedFilter = Preferences.getEditor("DefaultSaveFilter")
    ...
    def uncommentLine(self):
        ...
        if (Preferences.getEditor("CommentColumn0") and \
            not self.text(line).startsWith(commentStr)) or \
           (not Preferences.getEditor("CommentColumn0") and \
            not self.text(line).trimmed().startsWith(commentStr)):
            return
    ...
    def __autoSyntaxCheck(self):
        ...
        if Preferences.getEditor("AutoCheckSyntax"):
            ...
            if self.isPyFile():
                ...
                if syntaxError:
                    self.toggleSyntaxError(int(errorline), int(errorindex), 1, _error)
    ...
    def refresh(self):
        ...
        if not os.path.exists(unicode(self.fileName)):
    ...
    def editorCommand(self, cmd):
        ...
        if cmd == QsciScintilla.SCI_TAB:
            ...
            if not tmplName.isEmpty():
                if e4App().getObject("TemplateViewer").hasTemplate(
                        tmplName, self.getLanguage()):
                    self.__applyTemplate(tmplName, self.getLanguage())
                    return
                else:
                    templateNames = \
                        e4App().getObject("TemplateViewer").getTemplateNames(
                            tmplName, self.getLanguage())
                    if len(templateNames) == 1:
                        ...
                        self.showUserList(TemplateCompletionListID, 
                            QStringList(["%s?%d" % (t, self.TemplateImageID) \
    ...
    def __applyTemplate(self, templateName, language):
        ...
        if e4App().getObject("TemplateViewer").hasTemplate(
            ...
            e4App().getObject("TemplateViewer").applyNamedTemplate(
                tmplName, self.getLanguage())

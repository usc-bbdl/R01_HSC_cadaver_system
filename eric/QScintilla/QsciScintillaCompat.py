...
class QsciScintillaCompat(QsciScintilla):
    ...
    def monospacedStyles(self, font):
        ...
        for style in rangeLow + rangeHigh:
            self.SendScintilla(QsciScintilla.SCI_STYLESETFONT, style, f.toLatin1().data())
    ...
    def positionFromLineIndex(self, line, index):
        ...
        if QSCINTILLA_VERSION() >= 0x020700:
            # QScintilla >= 2.7 seems to cope with that already
            pos += index
        else:
    ...
    def __doSearchTarget(self):
        ...
        if self.isUtf8():
            targetSearchExpr = QString(self.__targetSearchExpr).toUtf8().data()
        else:
            targetSearchExpr = QString(self.__targetSearchExpr).toLatin1().data()

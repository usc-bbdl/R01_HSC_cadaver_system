...
class JavaScriptEricObject(QObject):
    ...
    translations = [
        QT_TRANSLATE_NOOP("JavaScriptEricObject", "Welcome to eric4 Web Browser!"), 
        QT_TRANSLATE_NOOP("JavaScriptEricObject", "eric4 Web Browser"), 
        QT_TRANSLATE_NOOP("JavaScriptEricObject", "Search!"), 
        QT_TRANSLATE_NOOP("JavaScriptEricObject", "About eric4"), 
    ]
    ...
    def providerString(self):
        ...
        return self.trUtf8("Search results provided by %1")\
            .arg(self.__mw.openSearchManager().currentEngineName())
...
class HelpBrowser(QWebView):
        ...
        if name.scheme().length() == 1 or \
           name.scheme() == "file":
            ...
            scriptSource = QUrl.fromPercentEncoding(name.toString(
                QUrl.FormattingOptions(QUrl.TolerantMode | QUrl.RemoveScheme)).toAscii())
    ...
    def saveAs(self):
        ...
        self.__unsupportedContent(reply, requestFilename = True, download = True)
    ...
    def __unsupportedContent(self, reply, requestFilename = None, download = False):
        ...
        if reply.error() == QNetworkReply.NoError:
            ...
            if reply.error() == QNetworkReply.OperationCanceledError and \
               reply.errorString() == "eric4:No Error":
                    return

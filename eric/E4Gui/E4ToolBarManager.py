...
class E4ToolBarManager(QObject):
    """
    ...
    def __toolBarByName(self, name):
        ...
        @param name object name of the toolbar (string or QString)
    ...
    def __findAction(self, name):
        ...
        @param name name of the action to search for (string or QString)
    ...
    def __findDefaultToolBar(self, name):
        ...
        @param name name of the default toolbar to search for (string or QString)
    ...
    def addToolBar(self, toolBar, category):
        ...
        @param category category for the toolbar (QString)
        """
        if toolBar is None:
            return
    ...
    def createToolBar(self, title, name=None):
        """
        Public method to create a custom toolbar.
        
        @param title title to be used for the toolbar (QString)
        @param name optional name for the new toolbar (QString)
    ...
    def deleteToolBar(self, toolBar):
        ...
        if self.__ui is not None:
            self.__ui.unregisterToolbar(unicode(toolBar.objectName()))
    ...
    def renameToolBar(self, toolBar, title):
        ...
        if self.__ui is not None:
            self.__ui.reregisterToolbar(unicode(toolBar.objectName()), title)
    ...
    def saveState(self, version = 0):
        ...
        stream.writeUInt16(E4ToolBarManager.VersionMarker)
        stream.writeUInt16(version)
        
        # save default toolbars
        stream.writeUInt16(E4ToolBarManager.ToolBarMarker)
    ...
    def restoreState(self, state, version = 0):
        ...
        if marker != E4ToolBarManager.VersionMarker or vers != version:
            return False
        
        tmarker = stream.readUInt16()
        if tmarker != E4ToolBarManager.ToolBarMarker:
            return False
        ...
        if cmarker != E4ToolBarManager.CustomToolBarMarker:
            return False
    ...
    def categoryActions(self, category):
        ...
        @param category category for the toolbar (string or QString)

...
class ProjectTranslationsBrowser(ProjectBaseBrowser):
        ...
        if itmList and not previewAll:
            for itm in itmList:
                if isinstance(itm, ProjectBrowserSimpleDirectoryItem):
                    dname = self.project.getRelativePath(unicode(itm.dirName()))
    ...
    def __readStdoutLupdate(self):
        ...
        proc = self.sender()
        if proc is not None:
            self.__readStdout(proc, '%s: ' % self.pylupdate)
        else:
            return
    ...
    def __readStderrLupdate(self):
        ...
        proc = self.sender()
        if proc is not None:
            self.__readStderr(proc, '%s: ' % self.pylupdate)
        else:
            return

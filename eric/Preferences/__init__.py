...
from Globals import settingsNameOrganization, settingsNameGlobal, settingsNameRecent, \
    isWindowsPlatform, getPyQt4ModulesDirectory
...
class Prefs(object):
    ...
    uiDefaults = {
        ...
        "VersionsUrls" : QtCore.QStringList() \
            << "http://die-offenbachs.homelinux.org:48888/eric/snapshots4/versions" \
    ...
    editorDefaults = {
        ...
        "DefaultOpenFilter" : QtGui.QApplication.translate('Lexers', 
                                'Python Files (*.py *.py3)'),
        "DefaultSaveFilter" : QtGui.QApplication.translate('Lexers', 
                                "Python Files (*.py)"),
        "AdditionalOpenFilters" : QtCore.QStringList(), 
        "AdditionalSaveFilters" : QtCore.QStringList(), 
        ...
        "CppDollarsAllowed" : 1, 
        "CppStylePreprocessor": 0, 
        "CppHighlightTripleQuotedStrings": 0,
        ...
        "PythonFoldQuotes": 0,
        "PythonStringsOverNewLineAllowed": 0,
        "PythonHighlightSubidentifier": 1,
    ...
    editorColourDefaults = {
        ...
        "EditAreaForeground"   : QtGui.QApplication.palette().color(
            QtGui.QPalette.Active, QtGui.QPalette.Base),
        "EditAreaBackground"   : QtGui.QApplication.palette().color(
...
def getUI(key, prefClass = Prefs):
    ...
    if key in ["BrowsersListFoldersFirst", "BrowsersHideNonPublic",
                ...
                "UseProxy", "ProxyPort", "ProxyType", "UseSystemProxy", 
...
def getEditor(key, prefClass = Prefs):
    ...
    if key in ["DefaultEncoding", "DefaultOpenFilter", "DefaultSaveFilter", 
        ...
        return prefClass.settings.value("Editor/" + key, 
            QtCore.QVariant(prefClass.editorDefaults[key])).toStringList()
    else:
        return prefClass.settings.value("Editor/" + key,
...
def getEditorKeywords(key, prefClass = Prefs):
    ...
    @param key the key of the value to get
    @param prefClass preferences class used as the storage area
    @return the requested list of language keywords (QStringList)
    """
    keywords = prefClass.settings.value("Editor/Keywords/" + key)
    if keywords.isValid():
        return keywords.toStringList()
    else:
        return QtCore.QStringList()
...
def setEditorKeywords(key, keywordsLists, prefClass = Prefs):
    ...
    @param key the key of the api to be set
    @param keywordsLists the list of language keywords (QStringList)
    @param prefClass preferences class used as the storage area
    """
    prefClass.settings.setValue("Editor/Keywords/" + key, QtCore.QVariant(keywordsLists))
...
def getMultiProject(key, prefClass = Prefs):
    ...
    @return the requested project setting
    """
    if key in ["OpenMasterAutomatically", "XMLTimestamp", "RecentNumber"]:
        return prefClass.settings.value("MultiProject/" + key,
            QtCore.QVariant(prefClass.multiProjectDefaults[key])).toInt()[0]
    else:
        return prefClass.settings.value("MultiProject/" + key,
            QtCore.QVariant(prefClass.multiProjectDefaults[key])).toString()
...
def getQt4DocDir(prefClass = Prefs):
    ...
    """
    s = unicode(prefClass.settings.value("Help/Qt4DocDir",
        QtCore.QVariant(prefClass.helpDefaults["Qt4DocDir"])).toString())
    if s == "":
        s = os.getenv("QT4DOCDIR", "")
    if s == "":
        s = unicode(QtCore.QLibraryInfo.location(QtCore.QLibraryInfo.DocumentationPath))
    return s
...
def getQt4TranslationsDir(prefClass = Prefs):
    ...
    """
    s = unicode(prefClass.settings.value("Qt/Qt4TranslationsDir", 
        QtCore.QVariant(prefClass.qtDefaults["Qt4TranslationsDir"])).toString())
    if s == "":
        s = os.getenv("QT4TRANSLATIONSDIR", "")
    if s == "":
        s = unicode(QtCore.QLibraryInfo.location(QtCore.QLibraryInfo.TranslationsPath))
...
def getQt(key, prefClass = Prefs):
    ...
    if key == "Qt4TranslationsDir":
        ...
        p = unicode(prefClass.settings.value("Qt/" + key, 
            QtCore.QVariant(prefClass.qtDefaults[key])).toString())
        if p == "":
            p = unicode(QtCore.QLibraryInfo.location(QtCore.QLibraryInfo.BinariesPath))
        return p
    else: 
        return prefClass.settings.value("Qt/" + key, 
...
def getTasks(key, prefClass = Prefs):
    ...
    if key in ["TasksColour", "TasksBugfixColour", 
        ...
        return prefClass.settings.value("Tasks/" + key,
            QtCore.QVariant(prefClass.tasksDefaults[key])).toInt()[0]
    else:
        return prefClass.settings.value("Tasks/" + key,
...
def getTrayStarter(key, prefClass = Prefs):
    ...
    return prefClass.settings.value("TrayStarter/" + key,
            QtCore.QVariant(prefClass.trayStarterDefaults[key])).toString()
...
def setTrayStarter(key, value, prefClass = Prefs):
    ...
    @param prefClass preferences class used as the storage area
    """
    prefClass.settings.setValue("TrayStarter/" + key, QtCore.QVariant(value))
